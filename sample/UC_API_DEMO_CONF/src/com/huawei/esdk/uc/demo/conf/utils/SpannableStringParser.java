...
import com.huawei.esdk.uc.demo.conf.widget.ClickableImageSpan;
...
{
	private static final String TAG = SpannableStringParser.class
			.getSimpleName();
	...
	public ImageSpan getEmotionSpan(String tagName)
	{
		...
		// {
		// Drawable drawable =
		// UCAPIApp.getApp().getResources().getDrawable(R.drawable.emotion01 +
		// index);
		// if (drawable != null)
		// {
		...
		try
		{
			...
			if (msg.getMediaType() == MediaResource.TYPE_NORMAL)
			{
				...
				showStr = ss == null ? strMessage : "";
					...
					&& msg.getMediaRes() instanceof JsonMultiUniMessage)
			{
	...
	/**
	 * 解析聊天界面显示的Span数据。 1 解析url 2 解析电话号码 3 解析表情
	 * 
	 * @param text
	 * @param isSend
	...
	{
		if (null == instantMsg
				|| TextUtils.isEmpty(instantMsg.getContent()))
		{
			tx.setText("");
			return false;
		}
		...
		if (instantMsg.getType() == Message.IM_FILE_TRANSFER)
		{
			...
			{
				content = LocContext.getContext().getString(
						R.string.um_file_sender_tip);
			} else
			{
				content = LocContext.getContext().getString(
						R.string.um_file_receiver_tip);
			}
			tx.setText(content);
			return false;
		}
	 ...
	 *            Is Conversation
	 */
	public void addFailTip(final TextView tx, final InstantMessage msg,
	...
	{
		if (msg == null
				|| !InstantMessage.STATUS_SEND_FAILED.equals(msg.getStatus()))
		{
			return;
		}
	 ...
	 */
	private SpannableString parseNumber(SpannableString ss, String content,
			final boolean isSelf)
	{
		...
		for (Tag t : listTag2)
		{
			...
			num = t.getTagStr();
			ss.setSpan(getClickableSpan(num, isSelf), t.getIndexBegin(),
					t.getIndexEnd(), Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
		}
		return ss;
	}

	private ClickableSpan getClickableSpan(final String num,
			final boolean isSelf)
	{
		ClickableSpan cs = new ClickableSpan()
		{
			...
			{
				ds.setColor(LocContext
						.getContext()
						.getResources()
						.getColor(
								isSelf ? R.color.primary
	 ...
	 * 解析网址
	 */
	private SpannableString parseUrl(SpannableString ss, String text,
			final boolean isSelf)
	{
		...
		for (Tag t : listTag)
		{
			if (ss.getSpans(t.getIndexBegin(), t.getIndexEnd(), ImageSpan.class) != null)
			{
				ss.removeSpan(ss.getSpans(t.getIndexBegin(), t.getIndexEnd(),
						ImageSpan.class));
			}
			String urlStr = parseHttpLowerCase(t.getTagStr());
			if (urlStr != null
					&& urlStr.toLowerCase(Locale.ENGLISH).startsWith("www."))
			{
				urlStr = "http://" + urlStr;
			}
			urlSpan = new URLSpan(urlStr)
			{
				...
				{
					if (!ContactLogic.getIns().getMyOtherInfo()
							.isEnableAnyOfficeBrowser())
					{
						try
						{
					...
					} else
					{
						Intent it = new Intent(
								"com.huawei.espace.anyofficeBrowser");
						it.putExtra("url", getURL());
						widget.getContext().startActivity(it);
					}
					// DialogUtil.showChooseBrowerDialog(widget.getContext(),
					// getURL());
				}
			};
	...
	private String parseHttpLowerCase(String tagStr)
	{
		...
		return tagStr.substring(0, index).toLowerCase(Locale.ENGLISH)
				+ tagStr.substring(index);
	}
	...
	public SpannableString parseEmotion(String text)
	{
		...
		for (Tag t : listTag)
		{
			if (null != urlTags && !urlTags.isEmpty())
			{
				...
				for (Tag tag : urlTags)
				{
					if ((t.getIndexBegin() >= tag.getIndexBegin())
							&& (t.getIndexEnd() <= tag.getIndexEnd()))
					{
						isInUrl = true;
						break;
					}
	 ...
	 */
	/*
	 * public boolean IsParseEmotion(String text) { List<Tag> urlTags =
	 * getTags(text, urlPattern); List<Tag> listTag = getTags(text,
	 * emotionPattern); if (listTag == null || listTag.isEmpty()) { return
	...
	public SpannableString parseInnerEmotionShow(String text)
	{
		...
		/*
		 * String strMessage=replaceBlank(text); if (strMessage != null) {
	...
	/**
	 * 方法名称：showChooseDialog 作者：xuchunping 方法描述：呼叫提取号码 输入参数:@param num 返回类型：void
	 * 备注：
	 */
