...
import com.huawei.esdk.uc.demo.conf.function.ConferenceFunc;
...
import com.huawei.esdk.uc.demo.conf.login.LoginActivity;
import com.huawei.esdk.uc.demo.conf.self.SelfInfoUtil;
...
public class UCAPIService extends Service
{
	...
	private BroadcastReceiver broadcastReceiver = new BroadcastReceiver()
	{
		...
		public void onReceive(Context context, Intent intent)
		{
			...
			// else if
			// (CustomBroadcastConst.ACTION_GET_GROUP_PIC.equals(action))
			// {
			...
			} else if (CustomBroadcastConst.CTC_PUSH_CONFINFO.equals(action))
			{
				BaseResponseData data = (BaseResponseData) intent
						.getSerializableExtra(UCResource.SERVICE_RESPONSE_DATA);
				Log.d("TTTTTTTTTTTTT01", data.toString());
			...
			} else if (CustomBroadcastConst.CTC_PUSH_DATACONF_INFO
					.equals(action))
			{
				BaseResponseData data = (BaseResponseData) intent
						.getSerializableExtra(UCResource.SERVICE_RESPONSE_DATA);
			...
			} else if (CustomBroadcastConst.CTC_UPDATE_CONF_STATUS_PUSH
					.equals(action))
			{
				BaseResponseData data = (BaseResponseData) intent
						.getSerializableExtra(UCResource.SERVICE_RESPONSE_DATA);
