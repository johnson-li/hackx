package com.huawei.esdk.uc.demo.conf.widget;
...
/**
 * 类名称：SlidingGroupView 类描述：可以平滑的一组视图
 */
public class SlippingViewGroup extends ViewGroup
{
	...
	/**
	 * 方法描述: 父view需要重新布局时,调用onMeasure. 备注: onMeasure is called when the parent
	 * View needs to calculate the layout. Typically, onMeasure may be called
	 * several times depending on the different children present and their
	 * layout parameters.
	 */
	@Override
	...
	/**
	 * 方法名称：snapToScreen 方法描述：移动到指定的屏幕 输入参数：@param destScreen 返回类型：void 备注：
	 */
	...
	/**
	 * 执行scroll后计算移动屏幕的位移和重新绘制屏幕 {@inheritDoc}
	 */
	@Override
	...
	/**
	 * 方法名称：getmCurrentScreen 方法描述：获取mCurrentScreen 返回类型：@return the
	 * mCurrentScreen 备注：
	 */
	...
	/**
	 * 方法名称：setmCurrentScreen 方法描述：设置当前页面的索引值.如果不是第一次执行布局,则跳到此页.
	 * 
	 * @param mCurrentScreen
	 *            the mCurrentScreen to set 返回类型：@return void 备注：
	 */
