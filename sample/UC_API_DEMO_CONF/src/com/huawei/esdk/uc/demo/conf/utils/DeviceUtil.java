...
public final class DeviceUtil
{
	...
	{
		Configuration config = LocContext.getContext().getResources()
				.getConfiguration();
	...
	public static void releaseKeepScreen()
	{
		...
		{
			Logger.debug(TAG,
					"screenLock.release exception, possible WakeLock under-locked ");
		}
	}

	/**
	 * 设置摄像头预览时的界面大小 要求： 预览界面的显示比率与要求比率一致（即长宽比)
	 * 
	 * @param camera
	 *            Camera对象
	 * @param ratio
			...
			boolean continuesPictures)
	{
		...
		if (null != sizes)
		{
			for (Camera.Size size : sizes)
			{
				...
				{
					Logger.debug(TAG, "height = " + size.height + "width = "
							+ size.width);
					...
					if (continuesPictures)
					{
						if ((supportModes != null
								&& supportModes
										.contains(Camera.Parameters.FOCUS_MODE_CONTINUOUS_PICTURE) && DeviceUtil
									.hasIceCreamSandwitch()))
						{
					...
					} else
					{
						...
						if ((supportModes != null && supportModes
								.contains(Camera.Parameters.FOCUS_MODE_CONTINUOUS_VIDEO)))
						{
							params.setFocusMode(Camera.Parameters.FOCUS_MODE_CONTINUOUS_VIDEO);
						} else if ((supportModes != null && supportModes
