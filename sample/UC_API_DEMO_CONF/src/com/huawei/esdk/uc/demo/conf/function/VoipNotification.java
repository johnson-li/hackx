...
import com.huawei.msghandler.conference.CallToConfHandler;
...
//			boolean isConf = tupCall.getIsFocus() == 1;
			boolean isConf = arg0.getIsFoces();
...
//			tupCall = session.getTupCall();
			if (session.isVideoCall())
			{
				...
				VoipFunc.getIns()
						.postSipNotification(VoipFunc.VIDEO_ADD_SUCESS);
				return;
			} else
			{
				if (VoipFunc.getIns().isVideo())
...
//		orientChange.setOrient(VideoFunc.transOrient(toOrient));
		 OrientChange orientChange = new OrientChange(VideoFunc.transOrient(toOrient));
	        orientChange.setOrient(VideoFunc.transOrient(toOrient));
		VoipFunc.getIns().sendBroadcast(VoipFunc.VIDEO_CHANGE_ORIENT,
				orientChange);
	}
