...
import com.huawei.esdk.uc.demo.im.adapter.PictureMainAdapter;
 ...
 */
public class PictureMainActivity extends BaseActivity implements
		View.OnClickListener, AdapterView.OnItemClickListener
{
	...
	public void initializeData()
	{
		...
		topicPicSelected = getIntent().getIntExtra(
				IntentData.TOPIC_PIC_SELECTED, 0);

		manager = SystemMediaManager.getIns().getRetriver(getContentResolver(),
				isVideo);
	}
	...
	private void choosePhotoOrVideoResult(Intent data)
	{
		...
		{
			// 将选择的图片添加到主预览页面中
			ArrayList<MediaRetriever.Item> select = (ArrayList<MediaRetriever.Item>) data
	...
	private void takePhotoOrVideoResult()
	{
		...
		if (isVideo)
		{
			ChatUtil.startVideoPlayerActivity(this, mediaPath, false,
					fromActivity, false);
			return;
		}
	 ...
	 *            choose
	 */
	private void startPictureScanActivity(String account, String oldPath,
			boolean choose)
	{
		...
		intent.putExtra(IntentData.STATUS,
				PictureScanActivity.COMMON_VIEW_UNSEND);
			...
			long id)
	{
		...
		MediaRetriever.Item directory = (MediaRetriever.Item) parent
