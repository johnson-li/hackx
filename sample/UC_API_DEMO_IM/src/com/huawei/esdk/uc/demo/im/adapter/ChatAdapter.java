package com.huawei.esdk.uc.demo.im.adapter;
...
import com.huawei.esdk.uc.demo.im.PictureScanActivity;
import com.huawei.esdk.uc.demo.im.R;
import com.huawei.esdk.uc.demo.im.UmImConstant;
import com.huawei.esdk.uc.demo.im.VideoPlayerActivity;
...
import com.huawei.esdk.uc.demo.im.headphoto.ContactHeadFetcher;
import com.huawei.esdk.uc.demo.im.utils.DeviceUtil;
import com.huawei.esdk.uc.demo.im.utils.DialogUtil;
import com.huawei.esdk.uc.demo.im.utils.ToastUtil;
import com.huawei.esdk.uc.demo.im.widget.CircleProgressBar;
import com.huawei.esdk.uc.demo.im.widget.ConfirmTitleDialog;
...
//		HeadPhotoUtil.getInstance().loadHeadPhoto(message.getFromId(), viewHolder.headImage);
		...
		viewHolder.mainText.setText(message.getNickname());
...
//    /**更换SDK后直接copy来的*/
//    private void previewFile(final MediaResource resource, final InstantMessage inMsg)
//    {
//        if (!(resource.getResourceType() == MediaResource.RES_URL))
//        {
//            startActivity(inMsg, resource);
//            return;
//        }
//        // 不在下载中，已经在下载中的没必要提示
//        if (!isTransFile(inMsg, resource, false))
//        {
...
//                }
//            };
//            if (preDownloadCheck(mContext, resource.getSize(), onClickListener))
//            {
//                return;
//            }
//        }
...
//    }
    ...
    private boolean preDownloadCheck(Context context, long fileSize,OnClickListener onClickListener)
    {
        return storageCheck() || DialogUtil.showDataLimitTip(context, fileSize, onClickListener);
    }
...
//    private void downloadMediaRes(InstantMessage im, MediaResource mediaRes)
//    {
//        if (!DeviceManager.isNetActive(mContext))
//        {
//            Logger.warn(TAG, "net error, not download thumbnail.");
//            return;
//        }
...
//    }
    ...
    {
    	/**更换SDK添加一个参数*/
//        return UmFunc.getIns().getProgress(im, mediaRes.getMediaId());
...
//            videoTransProgress(curVideo, im, mediaRes);
    ...
    private boolean showDataLimitTip(final InstantMessage im, final MediaResource mediaRes)
    {
        ...
        // 3G情况下提示size大小1M。
        if (mediaRes.getSize() > UmImConstant.DOWNLOAD_PROMPT_SIZE)
        {
    ...
    private void audioTransProgress(AudioMessageHolder holder, InstantMessage item)
    {
        ...
        {
            // 语音上传进度
            if (getProgress(im, res) == UmImConstant.NOT_DOWNLOAD)
            {
                // 发送失败的消息，显示发送失败重发按钮
...
//        }
    ...
    public void onTransFinish(UmReceiveData d, String transType)
    {
        ...
        if (d.status == UmReceiveData.FINISH_FAIL)
        {
            ...
            if (UmImConstant.UPLOADFILEFINISH.equals(transType))
            {
