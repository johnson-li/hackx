...
public class ContactDetailActivity extends Activity implements View.OnClickListener
{
    ...
    /** 逻辑类*/
    ContactDetailLogic contactDetailLogic;
    ...
    {
        contactDetailLogic = new ContactDetailLogic();

        Object object = getIntent().
                getSerializableExtra("CONTACT_DETAIL");

        if (object == null)
        {
            ActivityStack.getIns().popup(ContactDetailActivity.this);
            return;
        }

        currentContact = contactDetailLogic.getContact(object);
        if (currentContact == null)
        {
            Toast.makeText(ContactDetailActivity.this, "未获取到该联系人信息", Toast.LENGTH_SHORT).show();
