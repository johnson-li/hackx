...
{
	private static VideoFunction ins;
	...
	private VideoFunction()
	{
		videoCaps = new VideoCapsN();
		configVideoCaps();
	}

	public static VideoFunction getIns()
	{
		if (ins == null) 
		{
			ins = new VideoFunction();
		}
		return ins;
	}
	...
	{
		return VoipFunction.getInstance().closeVideo();
	}

	public boolean openVideo() 
	{
		return VoipFunction.getInstance().upgradeVideo();
	}
	...
	{
		VoipFunction.getInstance().agreeVideoUpgradte();
	}
    /**
     * 切换前后摄像头时,角度也会发生变化.
     */
...
//		videoCaps.setPlaybackLocal(ViERenderer.getIndexOfSurface(localVideo));
		...
		CallManager tempCallMan = VoipFunction.getInstance().getCallManager();
		...
		if (tempCallMan != null) 
		{
			...
			// else
			// {
			// tempCallMan.updateVideoWindow(0, videoCaps.getPlaybackRemote(),
			// sessionId);
			// }
		}
	...
	{
		CallManager manager = VoipFunction.getInstance().getCallManager();
		if (manager != null) 
		{
			manager.setOrientParams(videoCaps);
			// 设置融合会议，默认为false
			// manager.setCodecParams(false); 
		}
	}
